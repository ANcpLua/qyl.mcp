...
`/v2/protocol-versions.md`), so the SDK rules in `AGENTS.md` can be checked
against the documented behaviour offline. Documentation in the MCP project is
licensed CC-BY-4.0 (see the `LICENSE` shipped in `@modelcontextprotocol/server`).
