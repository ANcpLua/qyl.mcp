...
export const WORKBENCH_SESSION_COOKIE = "qyl-workbench-session";
...
interface StoredSession extends WorkbenchSessionIdentity {
    tokenHash: string;
    lastSeenMs: number;
    absoluteExpiryMs: number;
}

export interface CreatedWorkbenchSession {
    identity: WorkbenchSessionIdentity;
    setCookie: string;
}

export interface WorkbenchSessionManagerOptions {
    userId?: string;
    defaultWorkspaceId?: string;
...
export class WorkbenchSessionManager {
    ...
    constructor(options: WorkbenchSessionManagerOptions = {}) {
        ...
        if (this.idleTimeoutMs > this.absoluteTimeoutMs) {
            throw new Error("Workbench session idle timeout cannot exceed its absolute timeout.");
        }
        this.secureCookies = options.secureCookies ?? false;
        this.now = options.now ?? Date.now;
    }

    create(): CreatedWorkbenchSession {
        this.prune();
        const now = this.now();
    ...
    authenticate(cookieHeader: string | undefined): WorkbenchSessionIdentity | null {
        const token = readCookie(cookieHeader, WORKBENCH_SESSION_COOKIE);
...
function publicIdentity(session: StoredSession): WorkbenchSessionIdentity {
    return {
        id: session.id,
...
function boundedTimeout(value: number, name: string): number {
    if (!Number.isSafeInteger(value) || value < 1_000 || value > 31 * 24 * 60 * 60 * 1_000) {
        throw new Error(`Workbench session ${name} must be between one second and 31 days.`);
    }
    return value;
}
