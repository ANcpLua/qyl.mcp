...
export type EvaluationCaseStatus = WorkbenchEvaluationResultStatus;
...
export interface EvaluationCaseResult {
    ...
    executionId?: string;
    outcome?: ExecutionOutcome;
    assertionResults: readonly WorkbenchAssertionResult[];
    message?: string;
    usage?: InvocationUsage;
    traceId?: string;
    spanId?: string;
...
function sumTokenUsage(usages: readonly WorkbenchExecutionTokenUsage[]): WorkbenchExecutionTokenUsage {
    const sum = (select: (usage: WorkbenchExecutionTokenUsage) => number | undefined): number | undefined => {
        if (!usages.every((usage) => select(usage) !== undefined)) return undefined;
        return usages.reduce((total, usage) => total + select(usage)!, 0);
    };
    const inputTokens = sum((usage) => usage.input_tokens);
    const outputTokens = sum((usage) => usage.output_tokens);
    const totalTokens = sum((usage) => usage.total_tokens);
    return {
...
function sumCosts(costs: readonly WorkbenchExecutionCost[]): WorkbenchExecutionCost {
    const sources = new Set(costs.flatMap((cost) => cost.source === undefined ? [] : [cost.source]));
    return {
        amount_usd: costs.reduce((total, cost) => total + cost.amount_usd, 0),
...
export function exportEvaluationReport(run: EvaluationRun): string {
    ...
    const lines = [
        ...
        `- Cost: ${summary.cost === undefined ? "unavailable" : `$${summary.cost.amount_usd.toFixed(6)}${summary.cost.estimated ? " (estimated)" : ""}`}`,
        "",
        "## Test cases",
        "",
...
function formatTokens(summary: EvaluationSummary): string {
    ...
    const input = summary.tokenUsage.input_tokens === undefined ? "—" : String(summary.tokenUsage.input_tokens);
    const output = summary.tokenUsage.output_tokens === undefined ? "—" : String(summary.tokenUsage.output_tokens);
    const total = summary.tokenUsage.total_tokens === undefined ? "" : ` / ${summary.tokenUsage.total_tokens} total`;
