...
import {
    ...
    WorkbenchEvaluationExportPayloadSchema,
...
interface PersistedEvaluationExport {
    ...
    metadata: WorkbenchEvaluationExport;
...
const WorkspaceSchema: z.ZodType<WorkspaceRecord> = WorkspacePersistenceSchema.transform(
    (workspace) => WorkbenchWorkspaceSchema.parse(workspace),
);
...
const TestCaseSchema: z.ZodType<PersistedTestCase> = z.object({
    ...
    assertions: z.array(WorkbenchTestAssertionSchema).max(256),
...
const EvaluationRunSchema: z.ZodType<EvaluationRun> = z.object({
    ...
    status: WorkbenchEvaluationRunStatusSchema,
    ...
    confirmation: WorkbenchExecutionConfirmationEvidenceSchema.optional(),
    error: WorkbenchErrorSchema.optional(),
...
const PersistedEvaluationExportSchema: z.ZodType<PersistedEvaluationExport> = z.object({
    ...
    metadata: WorkbenchEvaluationExportSchema,
    payload: WorkbenchEvaluationExportPayloadSchema,
}).strict();
...
export class WorkbenchRepository {
    ...
    async initialize(): Promise<void> {
        ...
        const ownedWorkspaceIds = new Set<string>(
            state.workspaces
                .filter((workspace) => workspace.owner_id === this.ownerId)
    ...
    async createWorkspace(input: { name: string; description?: string }): Promise<WorkspaceRecord> {
        ...
        await this.store.transact((state) => {
            if (state.workspaces.some((item) => item.owner_id === this.ownerId && item.name === workspace.name)) {
...
function reconcileExecution(execution: PersistedExecution, completedAt: string): PersistedExecution {
    ...
    const recovered: ExecutionRecord = {
        ...
        error: {
            ...
            message: "Execution was interrupted by a workbench restart.",
...
function reconcileEvaluationRun(run: EvaluationRun, completedAt: string): EvaluationRun {
    ...
    return {
        ...
        error: {
            ...
            message: "Evaluation was interrupted by a workbench restart.",
            occurred_at: completedAt,
            retryable: true,
        },
    };
...
function validateState(state: WorkbenchState, ownerId: string): WorkbenchState {
    if (!state.workspaces.some((workspace) => workspace.owner_id === ownerId)) {
...
function assertWorkspaceDeletable(
    ...
    if (state.workspaces.filter((item) => item.owner_id === ownerId).length === 1) {
...
function findOwnedWorkspace(state: WorkbenchState, id: string, ownerId: string): WorkspaceRecord {
    const workspace = state.workspaces.find((item) => item.id === id && item.owner_id === ownerId);
