...
test("every published runner operation declares generated forbidden Problem Details", () => {
    ...
    for (const [path, pathItem] of Object.entries(qylOpenApi.paths)) {
        if (!path.startsWith("/runner/")) continue;
        for (const [method, operation] of Object.entries(pathItem)) {
            if (HTTP_METHODS.has(method)) operations.push({ method, path, operation });
        }
    }

    assert.ok(operations.length > 0, "published Qyl OpenAPI contains no runner operations");
