...
export type QylExecutionObservability = RunnerMcpExecutionTelemetryResponse;
...
function deriveTraceSignals(
...
): Pick<TelemetrySignalSummary, "exceptions" | "toolCallEvents"> {
    if (traceAvailability.status === "unavailable") {
        const reason = `Requires Qyl trace evidence: ${traceAvailability.unavailableReason ?? "traces unavailable"}`;
        return { exceptions: unavailableAvailability(reason), toolCallEvents: unavailableAvailability(reason) };
    }
    ...
    const reason = traceAvailability.unavailableReason;
    return {
        exceptions: derivedAvailability(status, exceptions, reason),
        toolCallEvents: derivedAvailability(status, toolCallEvents, reason),
    };
}
...
function unavailableAvailability(reason: string): TelemetrySignalAvailability {
    return { status: "unavailable", unavailableReason: boundedReason(reason), itemCount: 0 };
}
