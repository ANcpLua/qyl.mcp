...
test("workbench API authenticates, scopes, discovers, validates, and invokes idempotently", async () => {
    ...
    try {
        const denied = await fetch(`${harness.origin}/runner/workspaces`);
        assert.equal(denied.status, 401);
        assert.equal(denied.headers.get("www-authenticate"), "Cookie");

        const forged = await fetch(`${harness.origin}/runner/workspaces`, {
            headers: { cookie: "qyl-mcp-session=forged" },
        });
        assert.equal(forged.status, 401);

        const secondBootstrap = await fetch(`${harness.origin}/runner/session`, { method: "POST" });
        ...
        assert.match(secondCookieHeader ?? "", /Path=\/runner/u);
        assert.equal((await fetch(`${harness.origin}/runner/workspaces`, {
        ...
        const discovery = await getJson(
            harness,
            `/runner/workspaces/default/servers/${serverId}/discovery`,
        );
        ...
        const otherWorkspace = await postJson(harness, "/runner/workspaces", { name: "Other" });
        assert.equal(otherWorkspace.response.status, 200);
        const crossWorkspace = await getJson(
            harness,
            `/runner/workspaces/${String(otherWorkspace.body.id)}/servers/${serverId}`,
        );
...
test("workbench API exposes cancellation, timeout, and connection failure states", async () => {
    ...
    try {
        ...
        const cancellation = await postJson(
            harness,
            `/runner/workspaces/default/servers/${serverId}/executions/${delayedId}/cancel`,
        ...
        const missing = await postJson(harness, "/runner/workspaces/default/servers", {
        ...
        const failedConnection = await postJson(
            harness,
            `/runner/workspaces/default/servers/${missingId}/connect`,
            {},
        );
        assert.equal(failedConnection.response.status, 502);
        assert.equal(failedConnection.body.dependency, "mcp");

        const failedAutoConnect = await postJson(harness, "/runner/workspaces/default/servers", {
        ...
        const disconnected = await postJson(
            harness,
            `/runner/workspaces/default/servers/${serverId}/disconnect`,
            {},
        );
...
test("server configuration persistence failure preserves the connected runtime", async () => {
    ...
    const harness = await startHarness(join(directory, "state.json"));
    try {
        const created = await postJson(harness, "/runner/workspaces/default/servers", {
        ...
        try {
            const failed = await json(
                `${harness.origin}/runner/workspaces/default/servers/${serverId}`,
                {
                    method: "PATCH",
...
test("final-workspace deletion rejection preserves its live runtime registration", async () => {
    ...
    try {
        const serverId = await fixtureServerId(harness);
        const rejected = await json(`${harness.origin}/runner/workspaces/default`, {
            method: "DELETE",
        ...
        assert.equal(harness.workbench.connections.get(serverId).lifecycle, "connected");
        assert.equal(
            (await getJson(harness, `/runner/workspaces/default/servers/${serverId}`)).response.status,
            200,
        );
    } finally {
        await harness.close();
...
test("referenced-server deletion rejection leaves the server connected", async () => {
    ...
    try {
        const serverId = await fixtureServerId(harness);
        const created = await postJson(harness, "/runner/workspaces/default/test-cases", {
            serverId,
...
test("workbench SSE frames conform to the generated protocol and execution contracts", async () => {
    ...
    try {
        ...
        const protocolPayload = await firstSseData(
            `${harness.origin}/runner/workspaces/default/servers/${serverId}/protocol/stream`,
            harness.cookie,
        );
        assert.doesNotThrow(() => RunnerMcpProtocolEventSchema.parse(protocolPayload));

        const executionFrames = await readSseFrames(
            `${harness.origin}/runner/workspaces/default/servers/${serverId}/executions/stream?executionId=${executionId}`,
            harness.cookie,
            3,
        );
        const executions = executionFrames.map((frame) => RunnerMcpExecutionRecordSchema.parse(frame.data));
...
test("protocol SSE closes the snapshot race and resumes across reconnect generations", async () => {
    ...
    try {
        ...
        try {
            ...
            assert.doesNotThrow(() => RunnerMcpProtocolEventSchema.parse(raced?.data));
        } finally {
        ...
        const reconnect = await postJson(
            harness,
            `/runner/workspaces/default/servers/${serverId}/reconnect`,
            {},
        );
        ...
        assert.doesNotThrow(() => RunnerMcpProtocolEventSchema.parse(resumed?.data));
    } finally {
        await harness.close();
        await rm(directory, { recursive: true, force: true });
    }
...
test("execution SSE replays lifecycle cursors, closes the snapshot race, and survives reopen", async () => {
    ...
    try {
        ...
        const replayed = await readSseFrames(
            `${first.origin}/runner/workspaces/default/servers/${serverId}/executions/stream?executionId=${executionId}`,
            first.cookie,
            lifecycle.length - 1,
        ...
        try {
            const [racedFrame] = await readSseFrames(
                `${first.origin}/runner/workspaces/default/servers/${serverId}/executions/stream?executionId=${racedExecutionId}`,
                first.cookie,
                1,
            ...
            assert.doesNotThrow(() => RunnerMcpExecutionRecordSchema.parse(racedFrame?.data));
        } finally {
    ...
    try {
        const [persistedFrame] = await readSseFrames(
            `${reopened.origin}/runner/workspaces/default/servers/${serverId}/executions/stream?executionId=${racedExecutionId}`,
            reopened.cookie,
            1,
            { "Last-Event-ID": String(racedEventId - 1) },
        );
        ...
        assert.doesNotThrow(() => RunnerMcpExecutionRecordSchema.parse(persistedFrame?.data));
    } finally {
        await reopened.close();
        await rm(directory, { recursive: true, force: true });
    }
...
test("workbench API persists evaluations, comparisons, exports, and sanitized evidence across reopen", async () => {
    ...
    try {
        ...
        const referencedConnection = await postJson(first, "/runner/workspaces/default/servers", {
        ...
        const testCase = await postJson(first, "/runner/workspaces/default/test-cases", {
            serverId,
        ...
        const suite = await postJson(first, "/runner/workspaces/default/suites", {
        ...
        const baselineAccepted = await postJson(
            first,
            `/runner/workspaces/default/suites/${suiteId}/run`,
        ...
        const comparison = await postJson(first, "/runner/workspaces/default/evaluation-runs/compare", {
            baselineRunId,
            candidateRunId,
        });
        ...
        const jsonExport = await postJson(
            first,
            `/runner/workspaces/default/evaluation-runs/${baselineRunId}/export`,
            {
                format: "json",
        ...
        const jsonContent = await getJson(
            first,
            `/runner/workspaces/default/evaluation-runs/${baselineRunId}/exports/${jsonExportId}/content`,
        );
        ...
        const reportContent = await getJson(
            first,
            `/runner/workspaces/default/evaluation-runs/${baselineRunId}/exports/${reportExportId}/content`,
        );
        ...
        const preferences = await putJson(first, "/runner/workspaces/default/preferences", {
    ...
    try {
        assert.equal((await fetch(`${reopened.origin}/runner/workspaces`, {
            headers: { cookie: firstCookie },
        })).status, 401);
        assert.notEqual(reopened.cookie, firstCookie);

        const server = await getJson(reopened, `/runner/workspaces/default/servers/${serverId}`);
        assert.equal(record(server.body.connection).status, "connected");
        const persistedTest = await getJson(reopened, `/runner/workspaces/default/test-cases/${testCaseId}`);
        ...
        assert.equal((await getJson(reopened, `/runner/workspaces/default/suites/${suiteId}`)).response.status, 200);

        const persistedRun = await getJson(
            reopened,
            `/runner/workspaces/default/evaluation-runs/${baselineRunId}`,
        );
        ...
        const telemetry = await getJson(
            reopened,
            `/runner/workspaces/default/servers/${serverId}/executions/${executionId}/telemetry`,
        );
        ...
        const reopenedContent = await getJson(
            reopened,
            `/runner/workspaces/default/evaluation-runs/${baselineRunId}/exports/${reopenedExportId}/content`,
        );
        ...
        assert.equal((await getJson(
            reopened,
            `/runner/workspaces/default/evaluation-runs/${baselineRunId}/exports/${reportExportId}`,
        )).response.status, 200);

        const comparison = await postJson(reopened, "/runner/workspaces/default/evaluation-runs/compare", {
            baselineRunId,
            candidateRunId,
        });
        assert.equal(comparison.response.status, 200);
        const preferences = await getJson(reopened, "/runner/workspaces/default/preferences");
...
test("startup reconciles crashed executions and evaluations before idempotent replay", async () => {
    ...
    try {
        ...
        const recoveredEvaluation = await getJson(
            reopened,
            `/runner/workspaces/default/evaluation-runs/${evaluationRunId}`,
        );
...
test("concurrent evaluation idempotency coalesces one run and rejects payload conflicts", async () => {
    ...
    try {
        ...
        const path = `/runner/workspaces/default/suites/${suiteId}/run`;
...
async function fixtureServerId(harness: Harness): Promise<string> {
    const listed = await getJson(harness, "/runner/workspaces/default/servers");
...
async function waitForEvaluation(
    ...
    for (let attempt = 0; attempt < 200; attempt += 1) {
        const response = await getJson(
            harness,
            `/runner/workspaces/default/evaluation-runs/${runId}`,
        );
...
function cookieValue(setCookie: string | null): string {
    ...
    if (!cookie?.startsWith("qyl-mcp-session=")) throw new Error("Session cookie was not set.");
    return cookie;
}
