...
test("runner session uses HttpOnly SameSite cookie and returns metadata without the token", () => {
    ...
    assert.match(created.setCookie, /^qyl-mcp-session=/u);
    ...
    assert.doesNotMatch(JSON.stringify(created.identity), /qyl-mcp-session|token|cookie/iu);

    const identity = manager.authenticate(created.setCookie);
    assert.deepEqual(identity, created.identity);
});

test("runner session expires on idle or absolute timeout", () => {
    let now = 1_000_000;
    const manager = new RunnerSessionManager({
        idleTimeoutMs: 2_000,
...
test("revocation removes the session and returns an expired cookie", () => {
    ...
    assert.equal(manager.authenticate(created.setCookie), null);
    assert.match(cleared, new RegExp(`^${RUNNER_SESSION_COOKIE}=`));
    assert.match(cleared, /Max-Age=0/u);
});

test("cookie parsing handles multiple cookies and rejects malformed encoding", () => {
    assert.equal(readCookie("theme=dark; qyl-mcp-session=abc%20123; mode=dense", RUNNER_SESSION_COOKIE), "abc 123");
    assert.equal(readCookie("qyl-mcp-session=%E0%A4%A", RUNNER_SESSION_COOKIE), undefined);
    assert.equal(readCookie("qyl-mcp-session=", RUNNER_SESSION_COOKIE), undefined);
    assert.equal(readCookie(undefined, RUNNER_SESSION_COOKIE), undefined);
});

test("session timeout configuration is bounded", () => {
    assert.throws(() => new RunnerSessionManager({ idleTimeoutMs: 999 }), /between one second and 31 days/u);
    assert.throws(
        () => new RunnerSessionManager({ idleTimeoutMs: 5_000, absoluteTimeoutMs: 2_000 }),
        /cannot exceed/u,
    );
});
