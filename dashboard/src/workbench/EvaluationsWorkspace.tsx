...
import type {
  ...
  WorkbenchEvaluationSummary as EvaluationSummary,
...
function runLabel(run: EvaluationRun): string {
  return `${run.suite?.name ?? run.test_cases[0]?.name ?? "Evaluation"} · ${run.id.slice(0, 8)}`;
}

function summaryCards(summary: EvaluationSummary) {
  return [
    ["Success", formatPercent(summary.success_rate)],
...
function downloadArtifact(artifact: unknown, exportMetadata: EvaluationExport) {
  ...
  const blob = new Blob([content], { type: exportMetadata.media_type ?? (exportMetadata.format === "report" ? "text/markdown" : "application/json") });
  const objectUrl = URL.createObjectURL(blob);
  const link = document.createElement("a");
  link.href = objectUrl;
  link.download = (exportMetadata.file_name ?? `qyl-mcp-evaluation-${exportMetadata.run_id}.${exportMetadata.format === "report" ? "md" : "json"}`).replace(/[^A-Za-z0-9._-]/gu, "_");
  link.click();
  ...
  return (
    <div className="evaluations-workspace">
        ...
        <aside className="panel-surface run-list">
          ...
          {runs.map((run) => (
            <button type="button" key={run.id} className={`run-row${selectedRun?.id === run.id ? " is-selected" : ""}`} onClick={() => setSelectedRunId(run.id)}>
              <span><strong>{runLabel(run)}</strong><small>{formatTimestamp(run.created_at)}</small></span>
              <span className={`pill result-${run.status}`}>{run.status}</span>
              <span>{run.summary ? formatPercent(run.summary.success_rate) : "—"}</span>
            </button>
          ))}
            ...
            <>
              <section className="panel-surface evaluation-summary">
                <div className="section-heading-row">
                  <div><span className="section-kicker">{selectedRun.id}</span><h3>{runLabel(selectedRun)}</h3><p>{selectedRun.test_cases.length} immutable test snapshots · {formatTimestamp(selectedRun.started_at)} → {formatTimestamp(selectedRun.completed_at)}</p></div>
                ...
                {selectedRun.summary ? (
                  <>
                    ...
                    <div className="latency-strip">
                      <span>mean <strong>{formatDuration(selectedRun.summary.mean_duration_ms)}</strong></span>
                      <span>p50 <strong>{formatDuration(selectedRun.summary.p50_duration_ms)}</strong></span>
                      <span>p95 <strong>{formatDuration(selectedRun.summary.p95_duration_ms)}</strong></span>
                      <span>p99 <strong>{formatDuration(selectedRun.summary.p99_duration_ms)}</strong></span>
                      <span>tokens <strong>{selectedRun.summary.token_usage?.total_tokens ?? "—"}{selectedRun.summary.token_usage?.estimated ? " est." : ""}</strong></span>
                      <span>cost <strong>{selectedRun.summary.cost ? `$${selectedRun.summary.cost.amount_usd.toFixed(6)}${selectedRun.summary.cost.estimated ? " est." : ""}` : "—"}</strong></span>
                    </div>
                  </>
                ...
                {selectedRun.confirmation ? (
                  <details className="confirmation-evidence">
                    <summary>Explicit run approval retained · {formatTimestamp(selectedRun.confirmation.confirmed_at)}</summary>
              ...
              <section className="panel-surface result-evidence">
                ...
                {selectedRun.results.map((result) => <details className={`evaluation-result result-${result.status}`} key={result.test_case.id}><summary><span><strong>{result.test_case.name}</strong><small>{result.test_case.tool_name} · {formatDuration(result.duration_ms)}</small></span><span className={`pill result-${result.status}`}>{result.status}</span></summary><div className="evaluation-result-body">
                  {result.error ? <div className="inline-error"><strong>{result.error.category}</strong><span>{result.error.message}</span></div> : null}
                  <table className="data-table"><thead><tr><th>Assertion</th><th>Status</th><th>Message</th></tr></thead><tbody>{result.assertions.map((assertion) => <tr key={assertion.assertion_id}><td><code>{assertion.kind}</code></td><td><span className={`pill result-${assertion.status}`}>{assertion.status}</span></td><td>{assertion.message ?? "—"}{assertion.actual !== undefined ? <details><summary>Actual value</summary><JsonCodeView value={assertion.actual} label="Actual assertion value" /></details> : null}</td></tr>)}</tbody></table>
                  <JsonCodeView value={result.test_case} label="Immutable test snapshot" onCopy={copyText} />
                </div></details>)}
      ...
      <section className="panel-surface comparison-panel">
        ...
        {comparison ? <div className="comparison-results"><div className="metric-grid"><article><span>Success rate</span><strong>{formatDelta(comparison.success_rate_delta * 100, " pp")}</strong></article><article><span>Reliability</span><strong>{formatDelta(comparison.reliability_delta * 100, " pp")}</strong></article><article><span>p95 latency</span><strong>{formatDelta(comparison.p95_duration_delta_ms, " ms")}</strong></article><article><span>Tokens</span><strong>{formatDelta(comparison.token_delta)}</strong></article><article><span>Cost</span><strong>{formatDelta(comparison.cost_delta_usd, " USD")}</strong></article></div><table className="data-table"><thead><tr><th>Test</th><th>Regression</th><th>Baseline</th><th>Candidate</th><th>Duration Δ</th></tr></thead><tbody>{comparison.tests.map((test) => <tr key={test.test_case_id}><td><code>{test.test_case_id.slice(0, 8)}</code></td><td><span className={`pill regression-${test.status}`}>{test.status}</span></td><td>{test.baseline_status ?? "—"}</td><td>{test.candidate_status ?? "—"}</td><td>{formatDelta(test.duration_delta_ms, " ms")}</td></tr>)}</tbody></table></div> : <p className="empty-note">Comparison metrics appear only after the server evaluates two retained runs.</p>}
      </section>
    </div>
  );
}
